...
def plot_loss_accuracy(model_history, theme, path = None):
    '''This function will create a graph showing the change in loos throughout each epoch'''
    plt.style.use(theme)
    ...
    ax[0].set_ylabel('Loss (categorical_crossentropy)')
    ax[0].legend()
...
def plot_roc_auc(model, x_test, y_test, theme, model_type, path = None):
    ...
    # Finnaly average it and compute AUC
    mean_tpr /= n_classes
...
def plot_model_cm(test_cm, train_cm, classes, theme, model_type, cmap = plt.cm.Blues, path = None, normalize = False):
    ...
    for i, j in itertools.product(range(test_cm.shape[0]), range(test_cm.shape[1])):
        ax[0].text(j, i, format(test_cm[i, j], fmt), horizontalalignment = 'center', color = 'white' if test_cm[i, j] > thresh else 'black')
    ...
    for i, j in itertools.product(range(train_cm.shape[0]), range(train_cm.shape[1])):
        ax[1].text(j, i, format(train_cm[i, j], fmt), horizontalalignment = 'center', color = 'white' if train_cm[i, j] > thresh else 'black')
